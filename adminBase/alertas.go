...
func alertas() {
	db, err := sql.Open("postgres", "user=postgres host=localhost bdname=tarjetascredito sslmode=disable")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()

	fmt.Printf("Instalando Alertas ... \n")
	_, err = db.Exec(`
