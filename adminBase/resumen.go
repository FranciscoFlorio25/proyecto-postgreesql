...
func darResumen(){
	...
	defer db.Close()
	fmt.Printf(" Instalando fuciones de crecion de resumen ... \n")
	_, err =db.Exec(`
		...
		create or replace function resumen(nrocliente_buscado int, fechaDesde date, fechaHasta date) returns void as $$
			declare
			begin
				resumenPeriodo(nrocliente_buscado, fechaDesde, fechaHasta)
				 comprasDelPeriodo(nrocliente_buscado, fechaDesde, fechaHasta)
			end;
			$$ language plpgsql;

	fmt.Printf("Resumen instaladas ... \n")
	_, err = db.Exec(`
		create or replace function resume() returns trigger as $$
		begin
		insert into  (1);
	`)
}
func DarResumenFunciones(){
	darResumen()
	fmt.Printf("Funciones de creacion de resumen instaladas \n")
